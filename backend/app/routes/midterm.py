...
DATA_PATH = os.path.join(os.path.dirname(__file__), "..", "data", "midterm", "scenarios.json")
...
def get_state(iteration_id: int = 1):
    try:
        it = next(i for i in SCENARIOS["iterations"] if i["id"] == iteration_id)
        return {
...
def analyze_commentary(payload: CommentaryPayload):
    ...
    # Compact context sent to model — short, precise
    user = (
    ...
    try:
        text = analyst_feedback(system=sys, user=user)
